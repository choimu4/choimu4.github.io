...
// 모델의 프로퍼티 이름을 Swift 네이밍 컨벤션에 맞게 변경
struct MovieData: Codable {
    let boxOfficeResult: BoxOfficeResult
}
...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getData() {
        ...
        let task = session.dataTask(with: url) { data, _, error in
            ...
            do {
                self.movieData = try JSONDecoder().decode(MovieData.self, from: jsonData)
                DispatchQueue.main.async {
                    self.table.reloadData()
                }
            } catch {
...
// MyTableViewCell 클래스에 추가
extension MyTableViewCell {
    func configure(with movie: DailyBoxOffice) {
        movieName.text = "[\(movie.rank)위] \(movie.movieName)"
        audiAccumulate.text = "누적: \(formattedNumber(movie.audienceAccumulated))명"
        audiCount.text = "어제: \(formattedNumber(movie.audienceCount))명"
    }
